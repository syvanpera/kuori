...
| Voice typing counts voxtype's `streaming` as listening, and hides a `--no-osd` recording | The voice typing box itself began as this shell's own (the user's ask, 2026-09-25, over voxtype's floating card) and **the design adopted it** the same day, glyph, colours, `m:ss` and meter alike. What it does not know about is voxtype's: the design has only listening and transcribing, so a streaming session reads as listening, and `--no-osd` has no state in it at all. |
